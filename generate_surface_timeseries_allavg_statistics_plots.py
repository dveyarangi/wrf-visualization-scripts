...
def create_plots(time_groups, tag, config, domain_group, stations):
    ...
    for metrics_name in all_values.keys():
        ...
        for draw_param in ["wdir_deg", "wvel_ms", "temp2m_k", "rh2m"]:
            ...
            prefix = f'surface_timeseries_{config}_{metrics_name}_All Events_{domain_label}_{draw_param}_All Stations Avg'

            title = f"{draw_param.upper()} {domain_label} {metrics_name}, {config}, All Events&StationsAvg."
            print(" * " + title)
